 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Feathercoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Feathercoin and have many powerful features. Read
 ...
 *
 * <p>All Feathercoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    // These are serialized in both feathercoin and java serialization.
    ...
    public Sha256Hash getHash() {
        if (hash == null) {
            byte[] bits = feathercoinSerialize();
    ...
    /**
     * Used by FeathercoinSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Feathercoin share. In 2011 the value of a coinbase
    ...
    public String toString(AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(feathercoinValueToFriendlyString(out.getValue()));
                s.append(" BTC");
     ...
     * features of Feathercoin are developed.
     *
    ...
    synchronized Sha256Hash hashTransactionForSignature(int inputIndex, byte[] connectedScript,
        ...
        //
        //   https://en.feathercoin.it/wiki/Contracts
        ...
        try {
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // Feathercoin implementation. The reference client also contains a bug in handling this case:
    ...
    protected void feathercoinSerializeToStream(OutputStream stream) throws IOException {
        ...
        for (TransactionInput in : inputs)
            in.feathercoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.feathercoinSerialize(stream);
     ...
     * This is useful in certain types of <a href="http://en.feathercoin.it/wiki/Contracts">contracts</a>, such as
