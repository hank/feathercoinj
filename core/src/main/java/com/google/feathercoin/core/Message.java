...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Feathercoin proprietary serialization
...
public abstract class Message implements Serializable {
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = feathercoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Feathercoin protocol.
    ...
    /**
     * Should only used by FeathercoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by FeathercoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeFeathercoinSerialize()}, which is safe to mutate.
    ...
    public byte[] feathercoinSerialize() {
        byte[] bytes = unsafeFeathercoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the feathercoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeFeathercoinSerialize() {
        ...
        try {
            feathercoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to feathercoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the feathercoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void feathercoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use feathercoinSerialize().
     */
    void feathercoinSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented feathercoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so FeathercoinSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */
