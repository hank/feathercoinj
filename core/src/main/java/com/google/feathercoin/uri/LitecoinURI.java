...
/**
 * <p>Provides a standard implementation of a Feathercoin URI with support for the
 * following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code feathercoin:<address>}</li>
 * <li>{@code feathercoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://en.feathercoin.it/wiki/BIP_0021">BIP 0021</a>
 */
public class FeathercoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(FeathercoinURI.class);
    ...
    public static final String LITECOIN_SCHEME = "feathercoin";
    ...
    /**
     * Constructs a new FeathercoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws FeathercoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public FeathercoinURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws FeathercoinURIParseException If the input fails Feathercoin URI syntax and semantic checks.
     */
    public FeathercoinURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new FeathercoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  feathercoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form feathercoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the feathercoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : feathercoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("feathercoin://")) {
            schemeSpecificPart = input.substring("feathercoin://".length());
        } else if (input.startsWith("feathercoin:")) {
            schemeSpecificPart = input.substring("feathercoin:".length());
        } else {
            throw new FeathercoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new FeathercoinURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new FeathercoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a Feathercoin address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new FeathercoinURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new FeathercoinURIParseException("Malformed Feathercoin URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new FeathercoinURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The Feathercoin Address from the URI
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("FeathercoinURI[");
        boolean first = true;
    ...
    public static String convertToFeathercoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToFeathercoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Feathercoin URI builder using known good fields.
     * 
     * @param address The Feathercoin address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Feathercoin URI
     */
    public static String convertToFeathercoinURI(String address, BigInteger amount, String label, String message) {
        ...
        if (amount != null) {
            ...
            builder.append(Utils.feathercoinValueToPlainString(amount));
